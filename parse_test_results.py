    ...
    while line:
        ...
        if line.startswith("Finished "):
            if len(samples) != test_sample:
                raise ParsingError(
                    f"Repetition count does not match ({line_count}) in Test: {record['TestName']}"
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Parses results generated by any 'generate_test_script.py' script. "
        "Note that'DEBUG' must be 'False' in all tests"
    )
