...
def write_opening(test_case_dataset, fd):
    ...
    fd.write(
        f"docker network inspect {NETWORK_NAME} >{DEVNULL} 2>&1 || "
...
def write_closing(test_case_dataset, fd):
    ...
    for i in range(max_containers):
        fd.write(
            f"docker stop {IMAGE_NAME_FORMAT.format(i)} >{DEVNULL} && "
            f"docker rm {IMAGE_NAME_FORMAT.format(i)} >{DEVNULL}"
            "\n"
        )
    return max_containers
...
if __name__ == "__main__":
    ...
    if args.shell == "powershell7":
        ...
        sys.exit(f"Error: given shell '{args.shell}' is not supported")
